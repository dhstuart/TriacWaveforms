...
%     power_mins(i) = double(int(p,t,cut_in_times_V(i),pi)*(1/(pi-0))); %integrate power and evaluate between cut in time and pi   
...
for i = 1:length(vrms_mins_ratio)
    for j = 0:num_points
        dum=dum+1;
        power_values(dum) = ((pmax-power_mins(i))/num_points*j+power_mins(i))  %quartiles of power to test at
    end
end

unique_power_values = unique(power_values);
%% --------- determine voltage sine wave cut off corresponding to integrated power ---
guess = (1-power_values)*pi;

pwr_int = int(p,t,t,pi); %integral of power from t to pi


for i = 1:length(unique_power_values)
%         pwr_eqn = @(t)((1/2*(pi-sin(2*pi)/2)-1/2*(t-sin(2*t)/2))/(pi-0))-power_values(i);
%         cut_in_times_P(i) = fzero(pwr_eqn,guess(i));
    cut_in_times_P(i) = double(solve(pwr_int-unique_power_values(i)));
end
...
%     rms3(i) = double(rms2(cut_in_times_P(i)));
...
%         percent_area(dum) = ((1-vrms_mins(i))/num_points*j+vrms_mins(i));
                percent_area(dum) = ((1-vrms_mins_ratio(i))/num_points*j+vrms_mins_ratio(i));
    end
end
